...
async function fetchXContent(xAccounts, bearerToken, state, errors) {
  ...
  for (const account of xAccounts) {
    ...
    try {
      ...
      const allMedia = data.includes?.media || [];
      const allIncludedTweets = data.includes?.tweets || [];
      ...
      for (const t of allTweets) {
        ...
        if (t.attachments?.media_keys) {
          for (const mk of t.attachments.media_keys) {
            const m = allMedia.find((im) => im.media_key === mk);
            if (m) {
              media.push({
                type: m.type,
        ...
        // Extract quoted tweet content from expansions
        let quotedTweetContent = null;
        if (t.referenced_tweets) {
          const quotedRef = t.referenced_tweets.find((r) => r.type === "quoted");
          if (quotedRef) {
            const qt = allIncludedTweets.find((it) => it.id === quotedRef.id);
            if (qt) {
              quotedTweetContent = qt.note_tweet?.text || qt.text || null;
            }
          }
        }
